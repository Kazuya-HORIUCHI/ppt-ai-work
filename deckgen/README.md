# deckgen — セマンティック・スロット方式のスライド生成基盤

## 概要

本ディレクトリ（`deckgen/`）は、**スライドの中身（content）を宣言データとして書き、レンダラがレイアウト・装飾（presentation）を一手に引き受ける**構成のスライド生成基盤である。全発表に共通する基盤のみを置き、発表ごとの成果物（deck ファイル・生成 .pptx・アウトライン・発表固有の画像）はリポジトリ直下の `slides/<slug>/` 配下で管理する。
...
情報源 (md, ヒアリングメモ, 既存資料)
   ...
   生成 AI が deck.js を出力 ── 本ファイルで定義された kind 語彙を使う
        │
        ▼
   node deckgen/themes/<theme>/renderer.js deck.js out.pptx
        │
        ▼
   out.pptx
```
...
| `themes/<theme>/output/deck.example.js` | 当該テーマの全 kind を網羅するサンプル deck。書き方のリファレンスとしても使う（同ディレクトリにレンダリング済み `.pptx` を置く） |
...
- `themes/pastel/` — 淡色パレット + 画像スロット主体のテーマ。kind 語彙は basic と別系統（共通は `title-slide` / `section-divider` のみ）で、スキーマは `themes/pastel/output/deck.example.js` を参照する
...
```
node deckgen/themes/<theme>/renderer.js <deck.js のパス> <出力 .pptx のパス>
```
...
node deckgen/themes/basic/renderer.js slides/<slug>/deck-<slug>.js slides/<slug>/deck-<slug>.pptx
```
...
| kind | 用途 |
|---|---|
| [`cards`](#cards) | 2〜3 枚の本文カード。2 枚は対比、3 枚は並列観点（トレンド・観点等） |
...
全幅の箇条書きのみのスライド。コンテンツ領域全面にカードと同じ枠線なしのグレー背景が敷かれ、箇条書きはその内側に配置される（背景の大きさは items の量によらず固定）。
...
「これだけ持ち帰って欲しい」結論の一文だけを、中央に大きく強調する。
...
- 結論テキストは **36pt** で、コンテンツ領域中央の固定高ボックス（3 行ぶん = 1.95 in）に valign middle で表示される。行数が 1〜3 行のどれでもボックスの位置・高さは変わらない
- 1 行は全角換算 **約 22 文字**、`\n` 改行と自動折り返しを合わせて**最大 3 行**まで。超過するとボックスからあふれる
- 補足の箇条書きは持たない。補足が必要な内容は `bullets` 等の別スライドに分ける
...
2〜3 枚の本文カードを横並びで描画する。`cards.length` でレイアウトと既定 variant が切り替わる: 2 枚は左右の対比、3 枚は並列観点の横並び。カードの背景は枠線なし・直角のフラットな塗りで、variant は塗り色とタイトル色で表現される。

body を配列で渡した場合の箇条書きは **13pt**（`panel-bullets` と同じ）で、1 行あたりのピッチも `panel-bullets` の行高（0.55 in）に揃えている。
  ...
  cards: [
    {
      ...
      variant: "neutral" | "gray" | "pos" | "neg" | "accent",  // 省略時: 2 枚配置は neutral / 3 枚配置は gray
    },
    { /* 同上 */ },
  ],
}
...
- `cards.length` は **2 または 3**。範囲外の場合、カードは描画されず警告プレースホルダが表示される
- 4 観点以上に増やしたくなった場合は `data-table` か別構成を検討

**variant の選び方**

| variant | 色味 | 使い所 |
|---|---|---|
| `neutral` | 既定の薄グレー（タイトルだけアクセント色） | 2 枚配置でどちらも中立に並べる既定 |
| `gray` | 完全グレー（タイトルも本文色） | 並列観点をフラットに見せる（3 枚配置の既定） |
| `pos` | 緑系 | 良い面・成功例・推奨案 |
...
bullet 件数の上限は枚数によらず同じ（カード高の計算はカード幅に依存しない）。差が出るのは「1 bullet が 1 行で収まる文字数」で、カードが狭い 3 枚配置の方がシビア。

| 配置 | カード幅 | 1 カードあたり bullet 上限 | 1 bullet の文字数上限（折り返し回避） |
|---|---|---|---|
| 2 枚 | 約 5.87 in | **6 件**（推奨 3〜5 件） | 全角換算 **26 文字** |
| 3 枚 | 約 3.92 in | **6 件**（推奨 3〜5 件） | 全角換算 **15 文字** |

- 文字数上限を超えると 2 行折り返しでカード高がほぼ倍化する
...
各カードに `variant: "accent" | "pos" | "neg"` を指定するとタイトル帯と本文枠線の色が切り替わる。省略時は `accent`（テーマ青）。`pos` は緑、`neg` は赤で、`cards` の variant と同じ色味を用いる。
...
- `cards`: 2 主体の対比や 3 並列観点で、各カードに数件の bullet を載せる本文中心の構成
...
- `variant` は `accent` / `pos` / `neg` のみ。`cards` の `gray` 等は持たない
...
- 既定の `cardW` は `3.824 in`。これは 3 枚配置でカード間隔 `0.28 in` を取って算出した値で、`cards` 3 枚配置で測定された worst-case の 1 文字あたり描画幅 `0.192 in/全角` を使うと `floor(3.424 / 0.192) = 17` 字
...
- 1 カードあたり bullet 件数: **6 件まで**（推奨 3〜5 件）
- 1 bullet あたり文字数: 全角換算 **18 文字まで**（cards 2 枚配置の 26 字より厳しく、3 枚配置の 15 字より緩い）
...
- AI は画像ファイル自体を用意できないため、運用上はユーザが画像を `slides/<slug>/assets/` 等（deck.js と同じディレクトリ配下）に置いた上で、deck.js に相対パスを書く形になる
