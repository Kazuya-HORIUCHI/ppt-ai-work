...
情報源 (md / メモ / 既存資料)
        ...
        ▼
   slides/<slug>/deck-<slug>.js … 宣言データ。{ meta, sections, slides } を CommonJS export
        │
        │  node deckgen/themes/<theme>/renderer.js slides/<slug>/deck-<slug>.js out.pptx
        ▼
...
生成基盤は `deckgen/` 配下にまとまっており、以下のパスは断りが無い限り `deckgen/` からの相対で示す。発表ごとの成果物（deck ファイル・生成 .pptx・発表固有アセット）はリポジトリ直下の `slides/<slug>/` 配下に置く。
...
| `themes/<name>/output/deck.example.js` | 当該テーマの全 kind を網羅するサンプル deck。書き方のリファレンス兼、変更時の回帰確認用。レンダリング済み `.pptx` も同ディレクトリに置く |
| `slides/<slug>/deck-<slug>.js`（リポジトリ直下） | 生成された個別の deck（成果物）。発表固有の画像は同ディレクトリの `assets/` に置く |
| `.claude/skills/make-slides/SKILL.md`（リポジトリ直下） | 情報源 → deck.js → .pptx の生成ワークフロー定義 |
| `assets/samples/` | サンプル画像素材（basic の `output/deck.example.js` の `photo-card` が参照） |
...
6. `themes/<theme>/output/deck.example.js` に当該 kind のサンプルスライドを追加し、`node deckgen/themes/<theme>/renderer.js deckgen/themes/<theme>/output/deck.example.js deckgen/themes/<theme>/output/deck.example.pptx` で描画確認し、レンダリング済み `.pptx` を更新する。
...
4. 新テーマの全 kind を網羅する `themes/<name>/output/deck.example.js` を作成して描画し、全 kind の破綻有無を確認する。フォント・フォントサイズ・カード幅を変えた場合は文字数上限を再計測し、`charWMultiplier` 等のフィッティング値と README の制約値を新テーマ用に確定する。

テーマ間で deck.js 互換を保つことが最優先の制約である。スキーマを変えたくなった場合は、テーマ固有の分岐ではなく kind 語彙全体の変更として扱い、README と全テーマを同時に更新する。

### 7.3 挙動を変えないリファクタの検証手順

エンジン・テーマの内部整理では、変更前後で各テーマの `output/deck.example.js` を描画し、生成 .pptx を unzip して `ppt/` 配下を `diff -r` で比較する。スライド XML はレンダリングが決定的なため、挙動不変ならバイト一致する（`docProps/` はタイムスタンプを含むため比較対象から除外する）。
...
- **画像は事前に用意されたファイルのみ**: AI は画像を生成できないため、`photo-card` 等はユーザが `slides/<slug>/assets/` 等に置いたファイルへの相対パス参照（deck.js 基準）で運用する。SVG は非対応（PNG 変換が必要）。
