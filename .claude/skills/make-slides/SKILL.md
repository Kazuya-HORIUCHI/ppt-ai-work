...
|---|---|---|---|
| `cards` 2 枚 | 約 5.87 in | **8 件**（推奨は 4〜6 件） | 全角換算 **29 文字** |
| `cards` 3 枚 | 約 3.92 in | **8 件**（推奨は 4〜6 件） | 全角換算 **17 文字** |

注意点:

- **bullet が文字数上限を超えると 2 行に折り返し、高さがほぼ倍になる**。1 bullet が 2 行なら実質 2 件分の高さを取る。長い bullet を入れるなら件数を減らす（例: 3 枚配置で 30 字の bullet を 4 件入れたら、実質 8 件分の高さで上限超過）。
- 上限を超えそうな場合の対処（優先順）:
...
- カード間で bullet 数が大きく異なる場合（2 枚配置で 7 vs 3 等）も、**多い側を上限以下に保つ**こと。少ない側は問題にならない。
