...
カード系 kind（`comparison-2` / `trio`）は、カード高が「固定 Y を保てる上限」を超えると配置が中央寄せに自動切替される。さらにコンテンツ領域全体を超えると、フッターに重なって視認性が崩れる。

bullet 件数の上限は **両 kind とも同じ**（カード高の計算がカード幅に依存しないため）。差が出るのは **1 bullet あたりの文字数**で、カードが狭い方が折り返しやすい。

| kind | カード幅 | 1 カードあたり bullet 上限 | 1 bullet の文字数上限（折り返し回避） |
|---|---|---|---|
| `comparison-2` | 約 5.87 in | **8 件**（推奨は 4〜6 件） | 全角換算 **29 文字** |
| `trio` | 約 3.92 in | **8 件**（推奨は 4〜6 件） | 全角換算 **17 文字** |

注意点:

- **bullet が文字数上限を超えると 2 行に折り返し、高さがほぼ倍になる**。1 bullet が 2 行なら実質 2 件分の高さを取る。長い bullet を入れるなら件数を減らす（例: trio で 30 字の bullet を 4 件入れたら、実質 8 件分の高さで上限超過）。
- 上限を超えそうな場合の対処（優先順）:
  1. **bullet を短縮する**: 冗長な語を削る、句読点で 1 文に統合する
  2. **別 kind に変える**: 列挙が多いなら `data-table`、順序があるなら `flow-diagram` / `process-stages`、階層なら `pyramid`
  3. **スライドを分割する**: 同じ章内で 2 枚に割って粒度を上げる
- bullet を文字列ではなく**段落**にしたい（説明文を入れたい）場合は、`body` を配列ではなく文字列で渡す。ただしカード高は無闇に伸ばさないこと。
- 左右で bullet 数が大きく異なる場合（comparison-2 で 7 vs 3 等）も、**多い側を上限以下に保つ**こと。少ない側は問題にならない。
