...
| `SLIDE` | ページ骨格。スライド寸法、余白、タイトル / アクセント線 / メッセージ / コンテンツ領域 / フッターの Y 座標と高さ。**全 kind が共有するグリッドの基準** |
...
`addTitle`（タイトル + 実描画幅に追従するアクセント線 + メッセージ）、`addFooter`、`addUnimplementedPlaceholder`、`addBullets`（菱形マーカー `◆` 付き箇条書き）、`addCard` / `addCardRow` / `addTwoColRow`（角丸カードとその行揃え配置）、`addPanelCard(Row)` / `addPanelBulletsCard(Row)`（タイトル帯 + 区切り線つきパネル）、`addTable`（ヘッダ帯 + 縞模様の表）、`buildSectionDivider`、および各パーツの高さ計算（`cardHeight` / `panelCardHeight` / `panelBulletsCardHeight`）。
